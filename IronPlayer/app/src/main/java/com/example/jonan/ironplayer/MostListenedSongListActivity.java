...
public class MostListenedSongListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_most_listened_song_list);
        lv = (ListView)findViewById(R.id.mlsl);
        youtubeTrackPOJOList=new ArrayList<YoutubeTrackPOJO>();
        getMostListenedSongList();
        ...
        for (int i = 0; i < youtubeTrackPOJOList.size(); i++) {
            fruits_list.add(youtubeTrackPOJOList.get(i).getTitle()+" ("+youtubeTrackPOJOList.get(i).getListenedCounter()+")");
        }
    }

    public boolean getMostListenedSongList() {
        boolean rightsearchOnYoutube = true;

        String urlApiREST = Constants.URL_IRON_PLAYER_API_RESTFULL+"/playListService/getMostListenedSongList";
        try {
